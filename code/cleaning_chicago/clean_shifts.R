...
# working directory for shifts worked
# note: these are large files and not on github
# Toshio can share these files as needed
setwd("/Volumes/GoogleDrive-109693337169056844052/My Drive/Data/BWC/Shifts Worked v2 (with times)/raw FOIA data")
...
# separate first, last, and middle initial from name strings,
# some incorrect ones (e.g. Mc Nulty)
names_of_shifts <-
...
titles <-
  read_excel("P700490_Ferrazares_AAs-2014-2020.xlsx",
    sheet = unit_titles_sheets[2]
  ) %>%
  clean_names() %>%
  rename(title_cd = cd)
...
# but doing so by hand does not need worth it,
# just dropping all offenders for now
...
on_duty_shifts <- on_duty_shifts %>%
  ...
  group_by(id) %>%
  mutate(
    days_off = round(as.double(diff) / 86400) - 1,
...
# algorith checks if day worked number is missing
# if missing, take number of days worked from
# previous shift and add 1.
# Days worked is initialized at 1 for the first
# day of a work stretch from previous mutate.
on_duty_shifts <- on_duty_shifts %>%
  group_by(id) %>%
  arrange(date) %>%
  mutate(
    day_worked_number = ifelse(is.na(day_worked_number),
      lag(day_worked_number) + 1, day_worked_number
    ),
    day_worked_number = ifelse(is.na(day_worked_number),
      lag(day_worked_number) + 1, day_worked_number
    ),
    ...
    worked_gr_10_days = ifelse(is.na(day_worked_number), 1, 0)
  ) %>%
  ungroup()
